...
export class DashboardService {
          ...
          transactions: {
            ...
            others: {
              $subtract: [{
                $size: {
                  $filter:
                    { input: '$devices', as: 'item', cond: { $eq: ['$$item.status', 'WIDGET_PAYMENT'] } }
                }
              },
          ...
          operations: {
            ...
            others: {
              $subtract: [{
                $size: {
                  $filter:
                    { input: '$devices', as: 'item', cond: { $ne: ['$$item.status', 'WIDGET_PAYMENT'] } }
                }
              },
  ...
  async graphicsSales(stage: DashboardObject, insulation: Record<string, object>): Promise<object> {
    const startDate = new Date(Date.parse(stage.timeInterval.start));
    const endDate = new Date(Date.parse(stage.timeInterval.start));

    const distanceInDay = Math.ceil(Math.abs(new Date(stage.timeInterval.end).getTime() -
      new Date(stage.timeInterval.start).getTime()) / (1000 * 3600 * 24)) || 1
    ...
    for (let i = 1; i <= 12; i++) {
      endDate.setUTCHours((endDate.getUTCHours() + (days * 24 / 12)))

      const releaseEndDate = new Date(endDate);
      releaseEndDate.setMilliseconds(-1)
      const releaseStartDate = new Date(startDate)


      dates.push({
        start: releaseStartDate.toISOString(),
        end: releaseEndDate.toISOString()
      })

      injectOperations.push(
        {
          $size: {
            $filter: {
              input: '$operations',
              ...
              cond: {
                $and: [
                  { $gte: ['$$item.date', releaseStartDate] },
                  { $lte: ['$$item.date', releaseEndDate] }
                ]
              }
            }
          }
      ...
      startDate.setUTCHours(startDate.getUTCHours() + ((days * 24) / 12))
    }
    ...
    const request = await this.operationsSchema.aggregate([
      ...
      { $group: { _id: null, operations: { $push: '$$CURRENT' }, count: { $sum: 1 } } },
      {
        $project: {
          _id: false,
          operations: injectOperations,
          tickets: injectTickets,
          sales: injectSales, count: true,
          dates
        }
      }
    ]).exec()
